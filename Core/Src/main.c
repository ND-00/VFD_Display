...
char SSID[] = "HUAWEI-N1A2EI";
char Pass[] = "20142015Nick";
char ip_ntp[] = "1.pool.ntp.org";
...
int main(void)
{
  ...
  init_array();
  vfd_print("Инициализация...", 0, 0);
  HAL_Delay(5000);

  while(!esp_init())
	  vfd_display_error(1);

  vfd_print("Соедиенеие", 0, 5);
  ...
  {
	  vfd_clear();
	  vfd_print("Ошибка при получении времени(", 0, 0);
	  if(fail_counter++ > 5)
	  {
		  while(!esp_init())
			  vfd_display_error(1);
  ...
  while (1)
  {
	...
	if(flag_time_inc)
	{
		...
		sprintf(time_string, "%.2d %s %d Год",		current_time->tm_mday,
