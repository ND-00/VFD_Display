...
{
	if(Wifi_SendRaw((uint8_t*)data,strlen(data))==0x00)
		return 0x00;
	HAL_Delay(DelayUs);
	return 0x01;
}
...
uint8_t Wifi_WaitForString(uint32_t TimeOut_ms,uint8_t *result,uint8_t CountOfParameter,...)
{
	 ...
	 * received string the functions return a 0x01 value and the position of the
...
uint8_t Wifi_ReturnInteger(int32_t	*result,uint8_t WantWhichOne,char *SplitterChars)
{
	if((char*)Wifi.RxBuffer == NULL)
		return 0x00;
	if(Wifi_ReturnString((char*)Wifi.RxBuffer,WantWhichOne,SplitterChars)==0x00)
		return 0x00;
...
uint8_t	Wifi_Update(void)
{
	...
	do
	{
		...
		if(Wifi_SendString((char*)Wifi.TxBuffer)==0x00)
			break;
		if(Wifi_WaitForString(1000*60*5,&result,2,"OK","ERROR")==0x00)
			break;			// The timeout was completed and the string was not there
		if(result == 2)		// It was find the "ERROR" String in the receiving information
			break;
		returnVal=0x01;
...
uint8_t	Wifi_Station_DhcpIsEnable(void)
{
	...
	do
	{
		...
		if(result == 2)		// It was find the "ERROR" String in the receiving information
			break;
		if(Wifi_ReturnInteger((int32_t*)&result,1,":")==0x00)
...
uint8_t  Wifi_SoftAp_Create(char *SSID,char *password,uint8_t channel,WifiEncryptionType_t WifiEncryptionType,uint8_t MaxConnections_1_to_4,uint8_t HiddenSSID)
{
	uint8_t result;
	uint8_t	returnVal=0x00;
	do
...
uint8_t  Wifi_TcpIp_Ping(char *PingTo)
{
	...
	do
	{
		...
		if(Wifi_SendString((char*)Wifi.TxBuffer)==0x00)
			break;
		if(Wifi_WaitForString(_WIFI_WAIT_TIME_MED,&result,3,"OK","ERROR")==0x00)
			break;			// The timeout was completed and the string was not there
		if(result == 2)		// It was find the "ERROR" String in the receiving information
			break;
    if(Wifi_ReturnInteger((int32_t*)&Wifi.TcpIpPingAnswer,2,"+")==0x00)
      break;
		returnVal=0x01;
	}while(0);
	return returnVal;
...
uint8_t  Wifi_TcpIp_StartTcpConnection(uint8_t LinkId,char *RemoteIp,uint16_t RemotePort,uint16_t TimeOut)
{
	...
	do
	{
		...
		if(Wifi_SendString((char*)Wifi.TxBuffer)==0x00)
			break;
		if(Wifi_WaitForString(_WIFI_WAIT_TIME_HIGH,&result,3,"OK","CONNECT","ERROR")==0x00)
			break;
		if(result == 3)
			break;
		returnVal=0x01;
...
uint8_t  Wifi_TcpIp_SendDataTcp(uint8_t LinkId,uint16_t dataLen,uint8_t *data)
{
	...
	do
	{
		...
		if(Wifi_WaitForString(_WIFI_WAIT_TIME_LOW,&result,2,"OK","ERROR")==0x00)
			break;
		if(Wifi_WaitForString(_WIFI_WAIT_TIME_LOW,&result,3,">","ERROR","busy")==0x00)
			break;
		if(result > 1)
			break;
		Wifi_RxClear();
